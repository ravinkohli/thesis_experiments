...
xl_file = pd.ExcelFile('final_thesis_results/split_gathered_results.xlsx')
...
pd.concat([val for key, val in dfs.items() if 'mean' in key], axis=1).fillna(0).to_csv('final_thesis_results/combined_results_mean_(04.09).csv')
pd.concat([val for key, val in dfs.items() if 'std' in key], axis=1).fillna(0).to_csv('final_thesis_results/combined_results_std_(04.09).csv')
